...
struct tree_node *tree_find(struct tree_node *root, const struct bff_user_key *key)
{
	 ...
	 */
	 if ( root ) {
	 	if ( bff_user_cmp( key, &(root->data.key) ) > 0 ) { //key greater than root key
	 		return tree_find( root->right, key );
	 	}
	 	else if ( bff_user_cmp( key, &(root->data.key) ) < 0 ) { //key less than root key
...
struct tree_node *tree_delete(struct tree_node **p_root, const struct bff_user_key *key)
{
	...
	if ( node->left && node->right ) { //Node has two children
		struct tree_node *n = node->left;
		while( n->right ) {
			n = n->right;
		}
		n = tree_delete( p_root, &(n->data.key) );
		struct tree_node *node_return = malloc( sizeof (struct tree_node) );
		*node_return = *node;
		node->data = n->data;
		free(n);
		node_return->left = NULL;
		node_return->right = NULL;
		node_return->parent = NULL;
		
		return node_return;
	}
	else if ( node->left && !(node->right) ) { //Node only has left child
		if ( node == *p_root ) {
	...
	else if ( !(node->left) && node->right ) { //Node only has right child
		if ( node == *p_root ) {
	...
	else if ( !(node->left) && !(node->right) ) { //Node has no children
		if ( node != *p_root ) {
			if ( node == node->parent->right ) {
...
tree_data_t *tree_insert(struct tree_node **root, tree_data_t data)
{
	 ...
	 * the given root. Return the inserted struct bff_user pointer
	 */

	/* Fill in some code here. */
	if ( *root ) {
	 	if ( bff_user_cmp( &(data.key), &((*root)->data.key) ) > 0 ) { //key greater than root key
	 		struct bff_user *return_tmp = tree_insert( &((*root)->right), data );
	 		if ( (*root)->left ) {
	 	...
	 	else if ( bff_user_cmp( &(data.key), &((*root)->data.key) ) < 0 ) { //key less than root key
	 		struct bff_user *return_tmp = tree_insert( &((*root)->left), data );
	 		if ( (*root)->left ) {
	 			(*root)->left->parent = *root;
	 		}
	 		if ( (*root)->right ) {
...
{
	dist[edge_u][edge_v] = 1;
	dist[edge_v][edge_u] = 1;
	
	int i, j;
	for ( i = 0; i < user_count; i++ ) {
		for ( j = 0; j < user_count; j++ ) {
			if ( dist[i][j] > dist[i][edge_u] + 1 +  dist[edge_v][j] ) {
				dist[i][j] = dist[i][edge_u] + 1 +  dist[edge_v][j];
			}
			if ( dist[j][i] > dist[j][edge_v] + 1 +  dist[edge_u][i] ) {
				dist[j][i] = dist[j][edge_v] + 1 +  dist[edge_u][i];
			}
		}
	}
}
